...
public class UpdateWorld : MonoBehaviour
{
    ...
    void CheckNeigh(int x, int y)
    {
        for (int ny = -1; ny < 2; ny++)
        {
            for (int nx = 0 - 1; nx < 2; nx++)
            {
                CheckPoint(x + nx, y + ny);
            }
        }
    }

    private async Task CheckColor(int y)
    {
        for (int x = 0; x < texture.width; x++)
        {
            var count = CountNeighbourColor(x, y, Color.black, texture);
            var pixels = texture.GetPixel(x, y);
            ...
            else if (count > 0 && pixels == Color.white)
            {
                texture.SetPixel(x, y, Color.green);
            }
            //if near a black cell but the cell is already yellow
            //Mining Property
            if (count > 0 && pixels == Color.yellow)
            {
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out ray))
            {
                ...
                //CheckNeigh((int) lastPoint.x, (int) lastPoint.y);
                texture.Apply();
                 lastPoint = new Vector2((int)(ray.textureCoord.x * texture.width),
                                   (int)(ray.textureCoord.y * texture.height));
                 //CheckNeigh((int) lastPoint.x, (int) lastPoint.y);
